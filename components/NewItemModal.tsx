    ...
    try {
      ...
      const res = await fetch("/api/add-item", {
        ...
        body: JSON.stringify({
          ...
          // Code lassen wir komplett weg → wird serverseitig generiert
          billingAddress,
          contactName,
        }),
      })
          ...
          {tab === "form" && (
            <>
              ...
              <div className="rounded-2xl bg-black/20 p-4 space-y-3">
                <div className="text-[0.65rem] tracking-[0.2em] text-slate-400">
                  Auftrag
                </div>
                ...
                <div className="space-y-1">
                  <label className="block">
                    Ansprechpartner / Mieter (optional)
                  </label>
                  <input
                    type="text"
                    value={contactName}
              ...
              <div className="rounded-2xl bg-black/20 p-4 space-y-3">
                ...
                {/* Kunde */}
                <div className="space-y-1">
                  <label className="block">Kunde / Firma</label>
                  <input
                    type="text"
                    value={customerName}
                    onChange={(e) => setCustomerName(e.target.value)}
                    placeholder="z.B. Müller Rohrreinigung"
                    className="w-full rounded-xl border border-slate-600 bg-[#1f2125] px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="falls abweichend: Straße, PLZ Ort oder eigener Block"
                    className="w-full rounded-xl border border-slate-600 bg-[#1f2125] px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500"
                  />
                  <p className="text-[0.65rem] text-slate-400 normal-case">
                    Wenn leer, verwenden wir später einfach die Auftragsadresse
              ...
              {/* Block: Meta */}
              <div className="rounded-2xl bg-black/10 p-4 space-y-3">
                {/* Auftragsdatum */}
                <div className="space-y-1">
                  ...
                  />
                  <p className="text-[0.65rem] text-slate-400 normal-case">
                    Wenn leer, verwenden wir automatisch das heutige Datum.
                  </p>
                </div>

                <p className="text-[0.65rem] text-slate-400 normal-case">
                  Du kannst Felder auch erstmal leer lassen. Die Ampel steht dann
                  auf WARN, bis alle Stammdaten da sind.
                </p>
              </div>
            </>
          )}

          {/* WHATSAPP TAB (unverändert außer Platzhalter leicht angepasst) */}
          {tab === "whatsapp" && (
            <>
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder={`Beispiel:\nTyp: Auftrag\nKunde: Müller Rohrreinigung\nAdresse: Musterstr. 12, 12345 Ort\nDatum: 18.12.2025\nAP: Frau Meyer\nRechnungsadresse: Müller Rohrreinigung, Musterstr. 12, 12345 Ort`}
                  className="w-full rounded-xl border border-slate-600 bg-[#1f2125] px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500"
                />
                <p className="text-[0.65rem] text-slate-400 normal-case">
                  Später kommt das automatisch von WhatsApp. Für jetzt ist das unser
                  Test-Eingang.
                </p>
              </div>
            </>
