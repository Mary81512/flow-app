...
// Helfer: alle Video-Dateien holen
function getVideoFiles(files: File[]): File[] {
  return files.filter((f) => f.kind === "video")
}
      ...
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col gap-4 px-6 py-6">
        ...
        <section className="mt-6 flex-1">
          <div className="relative rounded-[3rem] bg-[#2f3238] px-10 py-8">
            ...
            {/* INHALT: Video-Liste oder Hinweis */}
            {videos.length === 0 ? (
              <div className="rounded-3xl bg-black/30 px-6 py-5 text-sm text-slate-200">
                Für diesen Auftrag sind noch keine Videos hinterlegt.
              </div>
            ) : (
